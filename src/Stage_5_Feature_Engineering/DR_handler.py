...
def main():
    df = pd.read_csv(global_conf.CSV_PATH)
    target_col = "label"  # change as needed
    ...
    print(f"Highly correlated columns: {len(correlated_cols) or 'None found'}")
    ...
    DR_Path = Path(global_conf.DR_REPORT_PATH)
    ...
    if model and hasattr(model, "explained_variance_ratio_"):
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(f"{DR_Path}/final_scree_plot.png")
        plt.close()
    ...
    if len(pc_cols) >= 2:
        ...
        plt.tight_layout()
        plt.savefig(f"{DR_Path}/final_class_separation.png")
        plt.close()
