...
class FeatureScalerTransformer(PerfMixin):
    def __init__(self, alpha: float = ALPHA, skew_thresh: float = 1.0, qt_max_rows: int = 100_000,
                 verbose: bool = True, report_file: str = "scaler_transform_report.csv"):
        self.alpha = alpha
    ...
    def _normality_scores(self, x: np.ndarray) -> Dict[str, float]:
        ...
        try:
            ...
            p_anderson = 0.25 if ad_stat.statistic < ad_stat.critical_values[2] else 0.01
        except Exception:
            p_anderson = 0.0
        ...
        result.update({
            ...
            "pass_rate": pass_rate,
            "passes_normality": pass_rate >= 0.66,  # require at least 2/3 tests pass
        })
        logging.info(
    ...
    def estimate_modes_and_variance(self, x: np.ndarray) -> Tuple[int, bool]:
        ...
        y_kde = kde(x_eval)
        peaks, _ = find_peaks(y_kde, height=np.max(
            y_kde) * 0.1, distance=len(x_eval) // 20)
        num_modes = len(peaks)
        ...
        if num_modes >= 3:
            ...
            km = KMeans(n_clusters=2, n_init=5,
                        random_state=42).fit(x.reshape(-1, 1))
            cluster_var = [np.var(x[km.labels_ == k]) for k in range(2)]
            overall_var = np.var(x)
            variance_ratio = np.mean(cluster_var) / \
                overall_var if overall_var > 0 else 0
            if variance_ratio < 0.5:
                structured_multimodal = True
        return num_modes, structured_multimodal

    def _detect_distribution(self, stats_dict: Dict[str, Any], data: np.ndarray) -> Tuple[str, float]:
        """
        ...
        """
        skew, kurt, entropy = stats_dict["skew"], stats_dict["kurtosis"], stats_dict["entropy"]
        dip_p = None
        ...
        if DIPO_AVAILABLE:
            try:
                ...
                dip_p = np.nan
                logging.warning(
                    "[DIPTest] DIP test failed; proceeding without it.")
        else:
        ...
        if dip_p is None or np.isnan(dip_p):
            if abs(skew) > 1.5 or entropy > ENTROPY_UNIFORM_THRESHOLD + 1 or abs(kurt) > 5:
                detected = "multimodal"
            else:
                detected = best_fit
        elif dip_p <= DIP_P_THRESHOLD:
            detected = "multimodal"
        elif abs(skew) < SKEW_THRESHOLD and KURTOSIS_THRESHOLD[0] < kurt < KURTOSIS_THRESHOLD[1] and dip_p > DIP_P_THRESHOLD:
        ...
        dip_p_str = f"{dip_p:.4f}" if dip_p is not None and not np.isnan(
            dip_p) else 'N/A'
        logging.info(
    ...
    def _plot_before_after(self, col: str, original: np.ndarray, transformed: np.ndarray):
    ...
    def _is_valid_for(self, method: str, x: np.ndarray) -> bool:
        ...
        if len(x_clean) < 3:
            logging.debug(
                f"[Validation] Too few non-NaN values for method '{method}'.")
            return False
        if np.isinf(x_clean).any():
            logging.debug(
                f"[Validation] Inf detected; method '{method}' not suitable.")
            return False
        ...
        if method == "boxcox":
            ...
            if np.any(np.isclose(x_clean, 0)):
                logging.debug(
                    "[Validation] Reciprocal invalid with zeros present.")
                return False
    ...
    def _evaluate_candidate(self, x: np.ndarray, method: str, scaler, scaler_name: str, col: str, original_entropy: float) -> Optional[Dict]:
        try:
            ...
            candidate_entropy = stats.entropy(
                np.histogram(x_clean, bins=30)[0] + 1)

            # Calculate entropy penalty ratio
            entropy_increase_ratio = (
                candidate_entropy / original_entropy) / np.log(30) if original_entropy > 0 else 1.0
            entropy_threshold = 2.0 if getattr(
                self, "structured_multimodal", False) else 1.5
    ...
    def _choose_fallback_scaler(self, x: np.ndarray, stats_dict: Dict[str, Any], num_modes: int, structured_multimodal: bool) -> Tuple[Any, str]:
        """
        ...
        if is_bounded:
            ...
            fallback_scaler, reason = RobustScaler(
            ...
            fallback_scaler, reason = StandardScaler(
    ...
    def _process_column(self, series: pd.Series, col: str):
        ...
        if len(x) < 50:
            ...
            num_modes, structured_multimodal = self.estimate_modes_and_variance(
                x)
        ...
        for method in self.extra_transforms + list(self.pre_funcs.keys()):
            ...
            candidate_scalers = [("robust", RobustScaler()), ("minmax", MinMaxScaler())] if structured_multimodal else \
                                [("standard", StandardScaler()), ("robust",
                                                                  RobustScaler()), ("minmax", MinMaxScaler())]
            for scaler_name, scaler in candidate_scalers:
                candidate = self._evaluate_candidate(
                    x, method, scaler, scaler_name, col, original_entropy)
                if candidate:
                    # Compute score
                    pval_score = 2 if candidate["harmonic_p"] > 0.05 else 1 if candidate["harmonic_p"] > 0.01 else 0
                    skew_score = 2 if abs(candidate["skew"]) < self.skew_thresh else 1 if abs(
                        candidate["skew"]) < self.skew_thresh * 1.5 else 0
                    entropy_ratio = (candidate["entropy"] / original_entropy) / \
                        np.log(30) if original_entropy > 0 else 1.0
                    entropy_score = 2 if entropy_ratio < 1.0 else 1 if entropy_ratio < 1.5 else 0
                    multimodal_penalty = 2 if structured_multimodal else 0
                    candidate["score"] = pval_score + skew_score + \
                        entropy_score - multimodal_penalty
        ...
        if candidates:
            candidates_sorted = sorted(
                candidates,
                key=lambda c: (-c["score"], abs(c['skew']), c['entropy'])
            )
            best_metrics = candidates_sorted[0]
            best_scaler, best_transformer = best_metrics['scaler'], best_metrics['transformer']
        else:
            ...
            best_metrics = {
                "method": "fallback_log1p+scaler" if best_transformer == self.pre_funcs['log1p'] else "fallback_scaler",
        ...
        if isinstance(best_transformer, (PowerTransformer, QuantileTransformer)) or \
            ...
            transformed_t = best_transformer.transform(
        ...
        passes_normality = final_normality["harmonic_p"] > self.alpha and abs(
            final_stats["skew"]) < self.skew_thresh
        passes_normality = final_normality["harmonic_p"] > relaxed_alpha and abs(
            final_stats["skew"]) < relaxed_skew_thresh

        pass_flag = "✅ Pass (post)" if passes_normality else "❌ Fail (post)" if not structured_multimodal else "❓ Ambiguous"

        self._plot_before_after(col, x, transformed.flatten())
        report_entry = {
            "original_mean": original_stats["mean"], "original_std": original_stats["std"],
            "original_skew": original_stats["skew"], "original_kurtosis": original_stats["kurtosis"],
            "original_entropy": original_stats["entropy"], "dip_pvalue": dip_p,
            "chosen_method": best_metrics["method"],
            "chosen_scaler": best_metrics["scaler_name"] if not already_normalizes else "skipped",
            "p_shapiro": best_metrics["p_shapiro"], "p_dagostino": best_metrics["p_dagostino"], "p_anderson": best_metrics["p_anderson"],
            "chosen_pval": best_metrics["harmonic_p"], "chosen_skew": best_metrics["skew"],
            "final_mean": final_stats["mean"], "final_std": final_stats["std"],
            "final_skew": final_stats["skew"], "final_kurtosis": final_stats["kurtosis"],
            "final_entropy": final_stats["entropy"], "num_modes": num_modes,
            "structured_multimodal": structured_multimodal, "post_pval": final_normality["harmonic_p"],
            "post_skew": final_stats["skew"], "pass_status": pass_flag,
        ...
        logging.info(f"[Decision] Column: {col} | Detected: {distribution_type} | Method: {best_metrics['method']} + {'skipped' if already_normalizes else best_metrics['scaler_name']} (final p={final_normality['harmonic_p']:.4f}, skew={final_stats['skew']:.3f})")
        return col, (best_scaler, best_transformer, best_metrics, candidates, report_entry)
    ...
    def fit(self, df: pd.DataFrame) -> 'FeatureScalerTransformer':
        ...
        for scaler in self.available_scalers:
            try:
                ...
                cols_passing = sum((np.array(pvals) > self.alpha) & (
                ...
                logging.warning(
                    f"[GlobalCheck] {type(scaler).__name__} failed: {e}")
    ...
    def transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.global_scaler_:
            logging.info(
                "[Transform] Applying global scaler to entire dataset.")
            transformed = self.global_scaler_.transform(
        ...
        for col in self.columns:
            ...
            if not np.any(nonnull_mask):
                logging.warning(
                    f"[Transform] Column '{col}' has all NaNs; skipping.")
                continue
            x_nonnull = x[nonnull_mask].reshape(-1, 1)

            best_transformer = self.transformers_[col]
            already_normalizes = (
                isinstance(best_transformer,
                           (PowerTransformer, QuantileTransformer))
                or (isinstance(best_transformer, tuple) and isinstance(self.scalers_[col], MinMaxScaler))
            )
            # Apply transformer if it exists
            if best_transformer:
                if isinstance(best_transformer, tuple) and best_transformer[0] == "boxcox":
                    # Box-Cox transformation using saved lambda
                    x_t = boxcox(x_nonnull.flatten(),
                                 lmbda=best_transformer[1])
                    already_normalizes = True
                    logging.info(f"[Transform] Applied Box-Cox on '{col}'.")
                elif isinstance(best_transformer, (PowerTransformer, QuantileTransformer)):
            ...
            if already_normalizes:
                ...
                transformed_col = self.scalers_[col].transform(
                    x_t.reshape(-1, 1)).flatten()
                logging.info(
    ...
    def generate_html_report(self, html_file: str = "scaler_transform_report.html") -> str:
        df = pd.read_csv(self.report_file)
        df["notes"] = np.where(df["chosen_method"].str.startswith(
            "fallback"), "Fallback applied", "OK")
        df["chart"] = df["feature"].apply(lambda f: f'<a href="{os.path.join(VISUALS_DIR, f"{f}_before_after.png")}" target="_blank">View</a>' if os.path.exists(
            os.path.join(VISUALS_DIR, f"{f}_before_after.png")) else "N/A")
        columns_to_keep = [
            "pass_status", "fallback_reason", "detected_distribution", "feature", "chosen_method", "chosen_scaler", "p_shapiro", "p_dagostino",
            "p_anderson", "chosen_pval", "chosen_skew",
            "original_mean", "original_std", "original_skew", "original_kurtosis",
            "original_entropy", "final_mean", "final_std", "final_skew", "final_kurtosis",
            "final_entropy", "dip_pvalue", "boxcox_lambda",
            "yeo_lambdas", "notes", "chart"
        ]
        df_simple = df[columns_to_keep]
        html_table = df_simple.to_html(
            index=False, border=0, classes="table table-striped table-hover", escape=False, float_format="%.4f")
        html_content = f"""
<!DOCTYPE html>
<html lang="en">
<head>
