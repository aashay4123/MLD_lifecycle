...
from sklearn.linear_model import LogisticRegression, Ridge, Lasso, LinearRegression, RidgeClassifier
...
from src.utils.utils import detect_task_and_search_space, get_cv, get_metric, cross_val_objective, compute_metric
...
class OptunaHyperTuner:
    ...
    def __init__(self, X, y=None, X_test=None, y_test=None,
        ...
        if self.task == "classification" and scoring not in ["roc_auc", "accuracy"]:
            raise ValueError(
                f"Incompatible scoring {scoring} for classification task")
        if self.task == "regression" and scoring not in ["r2", "neg_mean_squared_error", "neg_root_mean_squared_error"]:
            raise ValueError(
                f"Incompatible scoring {scoring} for regression task")
    ...
    def _auto_select_models(self):
        if self.task == "classification":
            return {
                "RandomForestClassifier": RandomForestClassifier(random_state=global_conf.RANDOM_STATE),
                "GradientBoostingClassifier": GradientBoostingClassifier(random_state=global_conf.RANDOM_STATE),
                "ExtraTreesClassifier": ExtraTreesClassifier(random_state=global_conf.RANDOM_STATE),
                "XGBClassifier": XGBClassifier(random_state=global_conf.RANDOM_STATE, eval_metric="logloss"),
                "LGBMClassifier": LGBMClassifier(random_state=global_conf.RANDOM_STATE, verbosity=-1),
                "CatBoostClassifier": CatBoostClassifier(random_state=global_conf.RANDOM_STATE, verbose=0),
                "SVC": SVC(probability=True),
                "LogisticRegression": LogisticRegression(max_iter=1000),
            }
        elif self.task == "regression":
            return {
                "RandomForestRegressor": RandomForestRegressor(random_state=global_conf.RANDOM_STATE),
                "GradientBoostingRegressor": GradientBoostingRegressor(random_state=global_conf.RANDOM_STATE),
                "ExtraTreesRegressor": ExtraTreesRegressor(random_state=global_conf.RANDOM_STATE),
                "XGBRegressor": XGBRegressor(random_state=global_conf.RANDOM_STATE),
                "LGBMRegressor": LGBMRegressor(random_state=global_conf.RANDOM_STATE, verbosity=-1),
                "Ridge": Ridge(),
                "Lasso": Lasso(),
    ...
    def evaluate(self, X_val, y_val):
        ...
        for model_name, (model, _) in self.best_models.items():
            test_score = compute_metric(
                y_val, model, X_val, metric_str)
            print(
                f"✅ {model_name} test {self.scoring}: {test_score:.4f}")
            if self.mlflow_run:
                mlflow.log_metric(
                    f"{model_name}_test_{self.scoring}", test_score)
    ...
    def tune(self):
        ...
        for model_name, model in self.models.items():
            ...
            num_pruned = sum(1 for t in study.trials if t.state
                             == optuna.trial.TrialState.PRUNED)
            print(f"📉 {num_pruned} trials were pruned for {model_name}.")
            print(
                f"✅ Best trial for {model_name}: {study.best_trial.value:.5f}")
    ...
    def _infer_direction(self):
        higher_is_better = "roc_auc" in str(
            self.scoring).lower() or "accuracy" in str(self.scoring).lower()
    ...
    def _generate_optuna_plots(self, model_name, study):
        ...
        for suffix, plot_func in plots:
            try:
                ...
                if self.mlflow_run:
                    mlflow.log_artifact(
                        f"reports/optuna/{model_name}_{suffix}.html")
            except Exception as e:
                print(
                    f"[WARN] Could not generate {suffix} plot for {model_name}: {e}")
